...
static void AddDummyBufferRW(ADD_FOR_PARAMS_ARGS_DEF) {
  assert(IsTapaType(param, "(i|o)buffers?"));

  // functions for dummy read/writes
  auto add_dummy_read_fifo = [&add_line](const std::string &name) {
    add_line("{ auto val = " + name + ".read(); }");
  };
  auto add_dummy_write_fifo = [&add_line](const std::string &name) {
    // write a default constructed instance of that type
    add_line(name + ".write(int());");
  };

  // TODO: Ensure that this works when buffer is partitioned,
  // as the dummy access only accesses the first element
  auto add_dummy_buffer_rw = [&add_line](const std::string &name) {
    add_line("{ volatile uint8_t val = reinterpret_cast<volatile uint8_t&>(" +
             name + ");");
    add_line("reinterpret_cast<volatile uint8_t&>(" + name + ") = val;}");
  };

  std::vector<std::string> names;

  if (IsTapaType(param, "(i|o)buffer")) {
  ...
  for (auto const &name : names) {
    const auto src_var = GetSrcVar(name);
    const auto sink_var = GetSinkVar(name);
    const auto data_var = GetDataVar(name);

    add_dummy_read_fifo(src_var);
    add_dummy_write_fifo(sink_var);
    add_dummy_buffer_rw(data_var);
  }
}
...
void XilinxHLSTarget::AddCodeForTopLevelBuffer(ADD_FOR_PARAMS_ARGS_DEF) {
  add_line("#error buffers not supported for top level task");
  return;
}
...
void XilinxHLSTarget::AddCodeForMiddleLevelBuffer(ADD_FOR_PARAMS_ARGS_DEF) {
  // TODO: At the moment, the middle level interface will always have a
  // dual-port core, no matter what, unless we add pipelined loops reading the
  // whole thing and even then partitioned memory will cause trouble. Better to
  // just ignore these wires in the stitching phase
  AddCodeForLowerLevelBuffer(ADD_FOR_PARAMS_ARGS);
  AddDummyBufferRW(ADD_FOR_PARAMS_ARGS);
}
...
void XilinxHLSTarget::AddCodeForLowerLevelBuffer(ADD_FOR_PARAMS_ARGS_DEF) {
  using partition_t = BufferConfig::partition_t;
  using partition_type_t = BufferConfig::partition_type_t;

  // TODO: Extend the code to adjust for array types
  assert(IsTapaType(param, "(i|o)buffers?"));
  const auto name = param->getNameAsString();
  add_pragma({"HLS disaggregate variable = ", name});

  auto isArrayType = IsTapaType(param, "(i|o)buffers") ? true : false;
  BufferConfig bufferConfig = ParseBufferType(param->getType(), isArrayType);
  std::vector<std::string> names;
  if (isArrayType) {
    add_pragma({"HLS array_partition variable =", name, "complete"});
    const int length = bufferConfig.length;
  ...
  for (auto const &name : names) {
    ...
    for (int i = 0; i < partition_scheme.size(); i++) {
      partition_t partition = partition_scheme[i];
      if (partition.first == partition_type_t::COMPLETE) {
        add_pragma({"HLS array_partition variable=", data_var, " type=complete",
                    " dim=", std::to_string(i + 2)});
      } else if (partition.first == partition_type_t::BLOCK) {
        add_pragma({"HLS array_partition variable=", data_var, " type=block",
                    " factor=", std::to_string(partition.second),
                    " dim=", std::to_string(i + 2)});
      } else if (partition.first == partition_type_t::CYCLIC) {
        add_pragma({"HLS array_partition variable=", data_var, " type=cyclic",
                    " factor=", std::to_string(partition.second),
                    " dim=", std::to_string(i + 2)});
      }
    }

    // add some dummy empty() and full() directions to let Vitis know the
    // direction of the FIFOs
    add_line("void(" + src_var + ".empty());");
    add_line("void(" + sink_var + ".full());");
  }
}
