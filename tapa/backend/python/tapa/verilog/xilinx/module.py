...
from tapa.codegen.buffergen import index_generator
...
class Module:
  ...
  def get_buffer_pipeline_level(self, buffer_name: str) -> int:
    return getattr(self, 'buffer_pipeline_level', {}).get(buffer_name, 1)
  ...
  def get_port_of_buffer(self, buffer: str, suffix: str) -> IOPort:
    """Return the IOPort of the given buffer with the given suffix.

    Args:
      buffer (str): Name of the buffer.
      suffix (str): One of the suffixes in IBUFFER_SUFFIXES or OBUFFER_SUFFIXES

    Returns:
      IOPort.
    """
    ports = self.ports
    sanitized_buffer = sanitize_array_name(buffer)
    port = ports.get(f'{sanitized_buffer}{suffix}')

    if port is None:
      # try finding by moving the array index
      match_object = match_array_name(buffer)
      if match_object is not None:
        base_name = match_object[0]
        index = match_object[1]
        arr = suffix.split('_')
  ...
  def make_buffer_simple(self, buffer_name: str) -> None:
    index = self._last_signal_idx + 1
    indices = []
    while index <= self._last_instance_idx:
      if self._module_def.items[index].instances:
        if self._module_def.items[index].instances[
            0].name == sanitize_array_name(buffer_name):
          lst = list(self._module_def.items[index].parameterlist)
          lst.append(
              ast.ParamArg(paramname='IS_SIMPLE', argname=ast.Constant('1')))
          self._module_def.items[index].parameterlist = tuple(lst)
  ...
  def add_buffer_instance(self, name: str, buffer_config: BufferConfig,
                          buffer_module_name: str,
                          make_simple: bool) -> 'Module':
    name = sanitize_array_name(name)
    rst_q = Pipeline(f'{name}__rst', level=self.register_level)
    self.add_pipeline(rst_q, init=ast.Unot(RST_N))
    ...
    def ports() -> Iterator[ast.PortArg]:
      ...
      # generate FIFO producer side ports
      producer_buffer_fifo_ports = buffer_config.get_producer_fifo_port_names()
      for port_name in producer_buffer_fifo_ports[:-2]:
        yield ast.make_port_arg(port=port_name,
                                arg=wire_name(name, f'{port_name}'))

      # set the `*_ce` signals to always TRUE
      yield ast.make_port_arg(port=producer_buffer_fifo_ports[-1], arg=TRUE)
      yield ast.make_port_arg(port=producer_buffer_fifo_ports[-2], arg=TRUE)

      # generate the FIFO consumer side ports
      consumer_buffer_fifo_ports = buffer_config.get_consumer_fifo_port_names()
      for port_name in consumer_buffer_fifo_ports[:-2]:
        yield ast.make_port_arg(port=port_name,
                                arg=wire_name(name, f'{port_name}'))

      # set the `*_ce` signals to always TRUE
      yield ast.make_port_arg(port=consumer_buffer_fifo_ports[-1], arg=TRUE)
      yield ast.make_port_arg(port=consumer_buffer_fifo_ports[-2], arg=TRUE)

      dims_patterns = buffer_config.get_dim_patterns()
      for index in index_generator(dims_patterns):
        for port_name in buffer_config.get_buffer_port_names():
          inner_port_name = port_name.format(index + 'producer_')
          outer_port_name = wire_name(name, inner_port_name)
          yield ast.make_port_arg(port=inner_port_name, arg=outer_port_name)
        for port_name in buffer_config.get_buffer_port_names():
          inner_port_name = port_name.format(index + 'consumer_')
          outer_port_name = wire_name(name, inner_port_name)
          yield ast.make_port_arg(port=inner_port_name, arg=outer_port_name)

    pipeline_level = self.get_buffer_pipeline_level(name)

    module_name = f'buffer_{buffer_module_name}'
    level = []
    if pipeline_level > 1:
      module_name = f'relay_buffer_{buffer_module_name}'
      level.append(
          ast.ParamArg(paramname='LEVEL', argname=ast.Constant(pipeline_level)))
    if make_simple:
      level.append(
          ast.ParamArg(paramname='IS_SIMPLE', argname=ast.Constant('1')))
    size_memcore = buffer_config.get_memcore_size()
    address_width = buffer_config.get_addr_width()

    return self.add_instance(
        module_name=module_name,
        instance_name=name,
        ports=ports(),
        params=(ast.ParamArg(paramname='MEMORY_DATA_WIDTH',
                             argname=ast.Constant(buffer_config.width)),
                ast.ParamArg(
                    paramname='MEMORY_ADDR_WIDTH',
                    argname=ast.Constant(address_width),
                ),
                ast.ParamArg(
                    paramname='MEMORY_ADDR_RANGE',
                ...
                ast.ParamArg(paramname='FIFO_ADDR_WIDTH',
                             argname=ast.Constant(
                                 max(1, (buffer_config.n_sections -
                                         1).bit_length()))),
                ast.ParamArg(paramname='FIFO_DEPTH',
                             argname=ast.Constant(buffer_config.n_sections)),
                ast.ParamArg(paramname='FREE_FIFO_RESET_LENGTH',
                             argname=ast.Constant(buffer_config.n_sections)),
                *level),
    )
  ...
  def cleanup(self) -> None:
    ...
    self.del_signals(suffix='_address0')
    self.del_signals(suffix='_we0')
    self.del_signals(suffix='_ce0')
