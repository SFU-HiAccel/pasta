...
from tapa.verilog.util import buffer_wire_name
...
class Task:
  ...
  def __init__(self, **kwargs):
    ...
    # contains BufferConfig objects that this task requires
    self.buffer_configs = {}
    # stores whether a buffer is simple or complex
    # simple buffer channels can be implemented via S2P memories while
    # complex buffer channels can be implemented via T2P memories
    self.buffer_simplicities = {}
    if self.is_upper:
      self.tasks = collections.OrderedDict(
      ...
      # objects
      for buffer_name, buffer_dict in self.buffers.items():
        self.buffer_configs[buffer_name] = BufferConfig(buffer_dict)
        if 'is_instantiated' in buffer_dict and \
            buffer_dict['is_instantiated'] == True:
          self.buffer_simplicities[buffer_name] = {
              'buffer_nature': False  # by default it's a complex
          }
  ...
  _DIR2CAT_FIFO = {'produced_by': 'ostream', 'consumed_by': 'istream'}
  _DIR2CAT_BUFFER = {'produced_by': 'obuffer', 'consumed_by': 'ibuffer'}

  def get_connection_to_fifo(
      self,
      fifo_name: str,
      direction: str,
  ) -> Tuple[str, int, str]:
    """Get port information to which a given FIFO is connected."""
    if direction not in self._DIR2CAT_FIFO:
    ...
    for port, arg in self.tasks[task_name][task_idx]['args'].items():
      if arg['cat'] == self._DIR2CAT_FIFO[direction] and arg['arg'] == fifo_name:
        return task_name, task_idx, port
    raise ValueError(f'task {self.name} has inconsistent metadata')

  def get_connection_to_buffer(
      self,
      buffer_name: str,
      direction: str,
  ) -> Tuple[str, int, str]:
    """Get port information to which a given buffer is connected."""
    if direction not in self._DIR2CAT_BUFFER:
      raise ValueError(f'invalid direction: {direction}')
    if direction not in self.buffers[buffer_name]:
  ...
  def get_buffer_directions(self, buffer_name: str) -> List[str]:
    directions = []
    for direction in ['consumed_by', 'produced_by']:
      if direction in self.buffers[buffer_name]:
        directions.append(direction)
    return directions
  ...
  def get_buffer_suffixes(self, direction: str) -> Dict[str, List[str]]:
    suffixes = {
        'consumed_by': {
            'fifo_suffixes': rtl.IBUFFER_FIFO_SUFFIXES,
            'memory_suffixes': rtl.IBUFFER_MEMORY_SUFFIXES
        },
        'produced_by': {
            'fifo_suffixes': rtl.OBUFFER_FIFO_SUFFIXES,
            'memory_suffixes': rtl.OBUFFER_MEMORY_SUFFIXES
        }
    }
    pass
  ...
  def is_buffer_external(self, buffer_name: str) -> bool:
    return 'is_instantiated' not in self.buffers[buffer_name]
  ...
  def connect_buffer_externally(self, internal_name: str,
                                buffer_config: BufferConfig) -> None:
    assert len(self.get_buffer_directions(internal_name)) == 1, \
        "externally connected buffers should have one direction"
    direction = self.get_buffer_directions(internal_name)[0]
    for suffix, width, wire_dir, intern_suffix, _ in buffer_config.get_fifo_suffixes(
        direction):
      rhs = buffer_wire_name(internal_name, intern_suffix)
      self.assign_directional(
          ast.Identifier(rtl.wire_name(internal_name, suffix[1:])),
          ast.Identifier(rhs), wire_dir)

    for index in index_generator(buffer_config.get_dim_patterns()):
      for suffix, width, wire_dir, intern_suffix, _ in buffer_config.get_memory_suffixes(
          direction):
        rhs = buffer_wire_name(internal_name, intern_suffix.format(index))
        self.assign_directional(
            ast.Identifier(
                rtl.wire_name(internal_name, suffix[1:].format(index))),
            ast.Identifier(rhs), wire_dir)
