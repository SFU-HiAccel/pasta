...
def create_parser() -> argparse.ArgumentParser:
  ...
  parser.add_argument(
      ...
      action='count',
      dest='separate_complex_buffer_tasks',
      help=
      'Recompile producer task when producer and consumer need to be separated '
      'and the producer needs recompilation. When disabled, they are put in '
      'the same slot')
...
def parse_steps(args, parser) -> Tuple[bool, str]:
  ...
  if args.enable_hbm_binding_adjustment:
    if not (_get_device_info(parser, args)['part_num'].startswith('xcu280') or _get_device_info(parser, args)['part_num'].startswith('xcu50')):
...
def main(argv: Optional[List[str]] = None):
  ...
  if all_steps or args.run_tapacc is not None:
    ...
    if args.input_file.endswith('.json') or args.work_dir is None:
      tapa_program_json = lambda: json.load(open(args.input_file, 'r'))
    else:
  ...
  if all_steps or args.run_floorplanning is not None:
    if args.floorplan_output is not None:
      ...
      separate_complex_buffer_tasks = True if args.separate_complex_buffer_tasks is not None else False
  ...
  if all_steps or args.generate_top_rtl is not None:
    program.generate_top_rtl(args.floorplan_output,
                             args.register_level or 0,
                             args.additional_fifo_pipelining,
                             _get_device_info(parser, args),
                             other_hls_config=args.other_hls_configs)
