...
class SlotManager:
  ...
  def getInitialSlot(self):
    ...
    ceil_to_multiple_of_x = lambda VAL, x : int(math.ceil(VAL/x))*x
    UR_X = ceil_to_multiple_of_x(self.board.CR_NUM_HORIZONTAL, 2) - 1
    UR_Y = self.board.CR_NUM_VERTICAL_PER_SLR*4 - 1
    
    if self.board.CR_NUM_HORIZONTAL == 8 and self.board.CR_NUM_VERTICAL == 8:
      return self.createSlot(f'CLOCKREGION_X0Y0:CLOCKREGION_X{7}Y{7}')
