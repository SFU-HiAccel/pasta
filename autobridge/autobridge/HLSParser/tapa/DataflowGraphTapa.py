...
class DataflowGraphTapa ():
  ...
  def __initEdges(self):
    ...
    for e_name, info in fifo_section.items():
      ...
      e.width = info['width']
      e.depth = info['depth']
      if e_name.find('BUFFER_EDGE') == 0:
        e.buffer_info['data_width'] = info['data_width']
        e.buffer_info['memcore_type'] = info['memcore_type']
        e.buffer_info['memcore_size'] = info['memcore_size']
        e.buffer_info['no_memcores'] = info['no_memcores']
        e.buffer_info['memcore_addr_width'] = info['memcore_addr_width']
        e.buffer_info['n_sections'] = info['n_sections']
        e.buffer_info['is_simple'] = info['is_simple']
        e.fifo_type = "BUFFER"
      self.edges[e_name] = e
