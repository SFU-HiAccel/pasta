...
def check_port_binding(config) -> None:
  ...
  if wrong_binding:
    ...
    logger.warning(
      'WARNING: '
      'Consider using --enable-hbm-binding-adjustment to allow automatic adjusting of the port binding '
      'based on the floorplan results. Or manually adjust the port binding to balance resources on the two sides of SLR 0.'
    )
    logger.warning('')
...
def is_device_supported(config) -> bool:
  ...
  supported_part_num_prefix = ('xcu280', 'xcu250', 'xcu200', 'xcu50')
